...
#if defined(CONFIG_MTK_FAN5405_SUPPORT) //jiangjingjing-add-for-pixi3-55
...
#endif//jiangjingjing-add-for-pixi3-55
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // Global Variable */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
...
#if defined(CONFIG_MTK_FAN5405_SUPPORT)  //jiangjingjing-add-for-pixi3_55//jiangjingjing-add-for-pixi3-55
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	//jiangjingjing-add-for-pixi3-55-20150418
...
#endif	//jiangjingjing-add-for-pixi3-55-20150418
...
//begin-20150518-jie.fang-add for MINISW Charger testitem show I_charging.
///////////////////////////////////////////////////////////////////////////////////////////
...
//end-20150518-jie.fang-add for MINISW Charger testitem show I_charging.
...
{	
	...
	//SCUD,: R_ID = 200k, Vbat_id=200/(200+24)*1.8V=1607mV, the valid gap is 1500~1700
	//BYD:    R_ID=21k, Vbat_id=10/(21+24)*1.8V=840mV, the valid gap is 750~950
...
{
/*modify by xiaopu.zhu for jrd battery warning policy*/
  if( BMT_status.charger_exist == KAL_TRUE )
    {
...
static void mt_battery_thermal_check(void)
{
	if ((g_battery_thermal_throttling_flag == 1) || (g_battery_thermal_throttling_flag == 3)) {
		...
		/* ignore default rule */
#else
  	//BEGIN: xiaopu.zhu modify  for importing the latest battery policy. 
	...
	//END: xiaopu.zhu modify  for importing the latest battery policy. 
	{
	...
	#else
			{
				if ((g_platform_boot_mode == META_BOOT)
					...
					power_supply_changed(bat_psy);
		    //BEGIN: add by xiaopu.zhu,MS has no display when charging highest and lowest temperature.
		    if(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
		    {
		       return;
		    }
		    //END: add by fxiaopu.zhu,MS has no display when charging highest and lowest temperature.
					if (BMT_status.charger_exist == KAL_TRUE) {
...
void BAT_thread(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
	 ...
	 //BEGIN: add by fangjie for  block shutdown beause of over temperature in MINI sw for task-137712
	#ifndef TARGET_BUILD_MMITEST
	mt_battery_thermal_check();
	#endif
	 //END: add by fangjie for  block shutdown beause of over temperature in MINI sw for task-137712
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	kal_uint32 icharging_enable = KAL_FALSE; //jianjingjing-add-for-pixi3-55
