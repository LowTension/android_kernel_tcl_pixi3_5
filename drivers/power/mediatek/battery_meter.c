...
//jiangjingjing-add-for-pixi3-55-20150428
#if defined ( CONFIG_PIXI3_55_SUPPORT) 
...
kal_uint32 g_fg_battery_id = 0;
kal_int32 g_Q_MAX_POS_50[] = {2405, 3004};
kal_int32 g_Q_MAX_POS_25[] = {2932, 3031};
kal_int32 g_Q_MAX_POS_0[]  = {2302, 3041};
kal_int32 g_Q_MAX_NEG_10[] = {2216, 2999};

kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {2390, 2985};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {2905, 3004};
kal_int32 g_Q_MAX_POS_0_H_CURRENT[]  = {1800, 2532};
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {1600, 1368};
...
{
	int g_bat_id =0;        
	g_bat_id = IMM_GetOneChannelValueEX(1,5);  

	bm_print(BM_LOG_CRTI, "~~~~~~~~battery id = %d jiale~~~~~~~~~\n",g_bat_id);             
	if(g_bat_id < 900) //742
	{
		   bm_print(BM_LOG_CRTI, "~~~~~~~~battery id = %d byd~~~~~~~~~\n",g_bat_id);             
		   g_fg_battery_id = 0;
	}
	else if(g_bat_id > 900) //930
	{
		   bm_print(BM_LOG_CRTI, "~~~~~~~~battery id = %d feimaotui~~~~~~~~~\n",g_bat_id);
		   g_fg_battery_id = 1;
	}
	else
	{
		   bm_print(BM_LOG_CRTI, "illegal battery\n");
	}
}
...
#if defined(BATTERY_DTS_SUPPORT) // add by xiaopu.zhu
#ifdef CONFIG_OF
...
//jiangjingjing-add-for-pixi3-55-20150521-begin
#if defined ( CONFIG_PIXI3_55_SUPPORT)
	return sizeof(battery_profile_t2[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUC);
#else
...
void oam_init(void)
{
	...
	kal_int32 g_rtc_fg_soc = 0;
	kal_int32 r_bootimg_vbat= 0;
//jiangjingjing-add-for-pixi3-55-20150428
//jiangjingjing-modify-for-charger-led-20150803
//don't let fan5405 control led by config reg00 as 80,only let isink3 control led.
      #if 0 //defined ( CONFIG_PIXI3_55_SUPPORT)
	int led_on = 0;
	...
	r_bootimg_vbat = fgauge_read_r_bat_by_v(g_booting_vbat);
	bm_print(BM_LOG_CRTI, "[oam_init] g_booting_vbat=%d, r_bootimg_vbat=%d,\n",
			 g_booting_vbat, r_bootimg_vbat);
	/*add by xiaopu.zhu for power on swocv offset*/
	#ifdef POWERON_BOOTIMG_VBAT_OFFSET
	g_booting_vbat=g_booting_vbat+POWERON_BOOTIMG_VBAT_OFFSET; //((400*r_bootimg_vbat)/1000);
	#endif
	/*add by xiaopu.zhu for power on swocv offset*/
      gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	...
	if (bat_is_charger_exist() == KAL_TRUE) {
		...
		if (((gFG_capacity_by_v == 100) && (vbat_capacity < batt_meter_cust_data.cust_poweron_max_vbat_tolrance))
		    || ((g_rtc_fg_soc==0)&&(abs(gFG_capacity_by_v - vbat_capacity) >batt_meter_cust_data.cust_poweron_delta_vbat_tolrance)) 
			||((g_rtc_fg_soc!=0)&&((abs(gFG_capacity_by_v - vbat_capacity) >batt_meter_cust_data.cust_poweron_delta_vbat_tolrance)||
