...
//#ifdef VELOCITY_CUSTOM
    ...
    else{//add by zq for tp compatible
    	tpd_load_status = 1;
    }
...
#endif
#if GTP_GESTURE_WAKEUP
    //jiangjingjing-move-to-event-handle-for-not-write-reg-to-provide power consumption increase-20151014
...
#if GTP_AUTO_UPDATE
	boot_mode = get_boot_mode(); //add by zoro.li 15.7.8
	if((boot_mode == NORMAL_BOOT)||(boot_mode == 11)) 
	{
...
#endif
static bool enDoubletap = KAL_FALSE;//jiangjingjing- add-for-doubletap-20151014-
static int touch_event_handler(void *unused)
{
...
//#if GTP_GESTURE_WAKEUP  jiangjingjing-delete
static s8 gtp_enter_doze(struct i2c_client *client)
{
    s8 ret = -1;
    s8 retry = 0;
...
static int __init tpd_driver_init(void)
{
	...
	printk("xutao boot mode is meta mode\n");
	return -1;	
    }
	printk("xutao boot mode is normal mode\n");
