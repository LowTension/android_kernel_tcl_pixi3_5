...
/*Begin-liuxin-Task863340 for ctp optimization-modify FT6x36 delaytime (10,10) to (100,30) according FAE's suggestion-task863340-20151105*/
...
/*End-liuxin-Task863340 for ctp optimization-modify FT6x36 delaytime according FAE's suggestion-task863340-20151105*/			
...
int fts_i2c_Read(struct i2c_client *client, u8 *writebuf,int writelen, u8 *readbuf, int readlen)
{
		...
		if(writelen!=0)
	{
		...
		ret=i2c_master_send(client, g_dma_buff_pa, writelen);
...

	{
		...
		{
		client->addr = client->addr & I2C_MASK_FLAG | I2C_DMA_FLAG;
		ret = i2c_master_recv(client, g_dma_buff_pa, readlen);
...
static  void tpd_down(int x, int y, int p) {
    ...
    /*Begin ersen.shang 20150914 for screen pinning reference defect 334816*/
	#if 0
	 ...
	 #endif
	 /*End   ersen.shang 20150914 for screen pinning reference defect 334816*/
...
static void check_gesture(int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_DOUBLECLICK:
		         //jiangjingjing- modify-for-doubletap-20151014-begin
				//input_report_key(tpd->dev, KEY_GESTURE_U, 1);
				input_report_key(tpd->dev, KEY_POWER, 1);
				input_sync(tpd->dev);
				//input_report_key(tpd->dev, KEY_GESTURE_U, 0);
				input_report_key(tpd->dev, KEY_POWER, 0);
                        //jiangjingjing- modify-for-doubletap-20151014-end
				input_sync(tpd->dev);
			break;
		case GESTURE_O:
...
static bool enDoubletap = KAL_FALSE;//jiangjingjing- add-for-doubletap-20151014-begin
 static int touch_event_handler(void *unused)
 {
	...
	u8 state;
	 do
	 {
		 ...
		 //jiangjingjing- add-for-doubletap-20151014-begin
                 if(Doubletap_Enable)
             	   {
             	       if (!enDoubletap)  //only enable double tap function one time.jiangjingjing
             	       {
...
reset_proc:   
 ...
 //   mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
	...
	if(uc_reg_value != 0x87 && uc_reg_value != 0x85)
	{
	...
	{
		tpd_vendor_id = CTP_VENDOR_JUNDA;
	}
	/*End   ersen.shang for add 3nd ctp LCE 20150910*/
...
#if 0//def FTS_GESTRUE  //jiangjingjing- delete-for-doubletap-20151014-begin 
      //not write relative reg in probe to provide power consumption increase       
	init_para(480,854,60,0,0);
