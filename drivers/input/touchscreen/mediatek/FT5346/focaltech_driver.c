...
reset_proc:
 ...
 //   tpd_mstar_status =0 ;  // compatible mstar and ft6306 chenzhecong
	...
	//fts_write_reg(i2c_client, 0xd0, 0x01);
      //jiangjingjing-delete-for-double-clicks,not write relative reg in probe to provide power consumption increase   
    //input_set_capability(tpd->dev, EV_KEY, KEY_POWER);
#endif
...
#ifdef FTS_GESTRUE
//jiangjingjing- add-for-doubletap-20151014-begin
	if(Doubletap_Enable)//if(true == TP_gesture_Switch)
	{
         fts_write_reg(i2c_client, 0xd0, 0x01);
