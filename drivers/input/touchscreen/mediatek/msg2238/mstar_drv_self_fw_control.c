...
/*Begin ersen.shang update firmware to v5.257*/ 
//#include "Pixi3-5_Each_Msg2238_V5.03_150313.h"
//#include "Pixi3-5_Each_Msg2238_V5.257_150430.h"
/*End   ersen.shang update firmware to v5.257*/

/*Begin ersen.shang update firmware to v5.202*/ 
//  #include "Pixi3-5_Each_Msg2238_V5.202_150511.h"
/*End   ersen.shang update firmware to v5.202*/ 

/*Begin shengrong.yuan update firmware to v5.203*/ 
//#include "PIXI3-5-3G_2238_Each_V0x5.203_20150522.h"
/*End   shengrong.yuan update firmware to v5.203*/ 

/*Begin shengrong.yuan update firmware to v5.204*/ 
//#include "PIXI3-5-3G_2238_Each_V0x5.204_20150526.h"
/*End   shengrong.yuan update firmware to v5.204*/ 

/*Begin xiaopu.zhu update firmware to v5.205 */ 
//#include "PIXI3-5-3G_2238_Each_V0x5.205_20150612.h"
/*End xiaopu.zhu update firmware to v5.205 */ 
/*Begin xiaopu.zhu update firmware to v5.206 */ 
//#include "PIXI3-5-3G_2238_Each_V0x5.206_20150616.h"
/*End xiaopu.zhu update firmware to v5.206 */ 

/*Begin xiaopu.zhu update firmware to v5.207 */ 
//#include "PIXI3-5-3G_2238_Each_V0x5.207_20150701.h"
/*End xiaopu.zhu update firmware to v5.207 */ 

/*Begin shengrong.yuan update firmware to v5.208 */ 
//#include "PIXI3-5-3G_2238_Each_V0x5.208_20150710.h"
/*End shengrong.yuan update firmware to v5.208 */ 
/*Begin xiaopu.zhu update firmware to v5.209 */ 
//#include "PIXI3-5-3G_2238_Each_V0x5.209_20150717.h"
/*End xiaopu.zhu update firmware to v5.209 */ 
/*Begin zhiquan.wen update firmware to v5.20a */ 
//#include "PIXI3-5-3G_2238_Each_V0x5.20A_20150811.h"//jiangjingjing- modify-for-doubletap-20151014
//#include "PIXI3_5_3G_2238_Each_V0x5.301_20150831.h"//jiangjingjing-modify-for-doubletap-20151014
/*End zhiquan.wen update firmware to v5.20a */ 
/*Begin 20151026 liujunting update firmware to v5.302*/ 
#include "PIXI3-5-3G_2238_Each_V0x5.302_20150921.h"
/*End 20151026 liujunting update firmware to v5.302*/
#include "msg2xxx_other_update_bin.h"
...
u32 g_GestureWakeupMode[2] = {0x00000001, 0x00000001};//jiangjingjing-modify-for-doubletap-20151014
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
//jiangjingjing- add-for-doubletap-20151014-begin
 if(Doubletap_Enable)
{   
    if (g_GestureWakeupFlag == 1)
    {
...
//jiangjingjing- add-for-doubletap-20151014-end
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
        ...
        */
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                else if (pPacket[5] == 5) // TOUCH_KEY_SEARCH 
                {	
                 pInfo->nTouchKeyCode = pPacket[5];
                }
                else
    ...
    else if (pPacket[nCheckSumIndex] == nCheckSum && pPacket[0] == 0x62)
    {
        ...
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[5] == 0xFF) && (pPacket[6] == 0xFF) && (pPacket[7] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                else if (pPacket[8] == 5) // TOUCH_KEY_SEARCH 
                {	
			pInfo->nTouchKeyCode = pPacket[5];
                }
                else
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        ...
        {
            nUpdateBinMajor = msg2xxx_other_update_bin[0xBFF5]<<8 | msg2xxx_other_update_bin[0xBFF4];
            nUpdateBinMinor = msg2xxx_other_update_bin[0xBFF7]<<8 | msg2xxx_other_update_bin[0xBFF6];
        }
            ...
            if (eSwId < MSG22XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF)
            {
                ...
                else if (eSwId == MSG22XX_SW_ID_YYYY)
                {
                    for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                    {
                        if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_other_update_bin[i*1024]), 1024);
                        }
                        else // i == 48
                        {
                            _DrvFwCtrlStoreFirmwareData(&(msg2xxx_other_update_bin[i*1024]), 512);
                        }
                    }
                }
        ...
        if ((nUpdateBinMinor > nMinor && nIsCompareVersion == 1) || (nIsCompareVersion == 0))
        {
            if (eSwId < MSG21XXA_SW_ID_UNDEFINED && eSwId != 0xFFFF)
            {
                ...
                else if (eSwId == MSG21XXA_SW_ID_YYYY)
                {
                    ...
                    {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                        _DrvFwCtrlStoreFirmwareData(msg2xxx_other_update_bin[i], 1024);
#else // By one dimensional array
                        _DrvFwCtrlStoreFirmwareData(&(msg2xxx_other_update_bin[i*1024]), 1024);
#endif
                    }
...
static bool setDoubletap = KAL_FALSE;//jiangjingjing-add-20151014
...
//jiangjingjing- add-for-doubletap-20151014-begin
 if(Doubletap_Enable)
{
    if (!setDoubletap)  //only enable double tap function one time.jiangjingjing
    {             	       
	setDoubletap=true;             	     
...
//jiangjingjing- add-for-doubletap-20151014-end
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        if ((tInfo.nFingerNum) == 0)   //touch end
        {
            if (nLastKeyCode != 0)
            {
                DBG("key touch released  nTouchKeyCode=%d,nLastKeyCode=%d\n",nTouchKeyCode,nLastKeyCode);

    		    if (key_back_menu == nLastKeyCode)//add by xiewei@tcl
    		    {
        ...
        else //touch on screen
        {
            if (tInfo.nTouchKeyCode != 0)
            {
	            ...
	            else if (tInfo.nTouchKeyCode == 5)//for TOUCH_KEY_MENU+TOUCH_KEY_BACK xiewei
	            {
                ...
                if (nLastKeyCode != nTouchKeyCode)
                {
					...
					else if(nLastKeyCode==key_back_menu)
					{
					    two_key_rel = 5&tInfo.nTouchKeyCode;
					    if(two_key_rel==1)    //menu release
					    {
							input_report_key(g_InputDevice, KEY_MENU, 0);
							input_sync(g_InputDevice);
					    }
						else if(two_key_rel==4)    //back relaese
						{
						    input_report_key(g_InputDevice, KEY_BACK, 0);
							input_sync(g_InputDevice);
						}
