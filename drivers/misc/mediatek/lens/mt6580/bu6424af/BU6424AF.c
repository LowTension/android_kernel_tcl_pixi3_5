...
static struct i2c_board_info kd_lens_dev __initdata = { I2C_BOARD_INFO("BU6424AF", 0x20) };
...
static int s4BU6424AF_ReadReg(unsigned short *a_pu2Result)
{
  ...
  BU6424AFDB("zejia[BU6424AF] I2C success!\n"); 
	return 0;
}
...
inline static int moveBU6424AF(unsigned long a_u4Position)
{
	...
	spin_unlock(&g_BU6424AF_SpinLock);
	 ...
	 BU6424AFDB("[BU6424AF] move [curr] %ld [target] %ld\n", g_u4CurrPosition, g_u4TargetPosition); 
	...
	if (s4BU6424AF_WriteReg((unsigned short)g_u4TargetPosition) == 0) {
		...
		spin_unlock(&g_BU6424AF_SpinLock);
		BU6424AFDB("zejia[BU6424AF] I2C success when moving the motor\n"); 
	} else {
