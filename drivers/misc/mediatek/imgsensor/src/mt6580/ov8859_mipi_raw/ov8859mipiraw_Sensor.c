...
#include "ov8859mipiraw_Sensor.h"
...
OV8859_VERSION ov8859version = OV8859R2A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8859_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	
	.checksum_value = 0xd14be45,		//checksum value for Camera Auto Test
	
	.pre = {
		.pclk = 74400000,				//record different mode's pclk
		.linelength  = 1952,				//record different mode's linelength
	...
	.cap_delay_frame = 1,		// old:3 enter capture delay frame num  jiangjingjing-modify-for-pixi3-55-goole-cts-test-Task:340217-20150612
	.pre_delay_frame = 1, 		//old:3 enter preview delay frame num   jiangjingjing-modify-for-pixi3-55-goole-cts-test-Task:340217-20150619
	.video_delay_frame = 1,		//old:3 enter video delay frame num   jiangjingjing-modify-for-pixi3-55-goole-cts-test-Task:340217-20150619
...
static void r1asensor_init(void)
{
	LOG_INF("E\n");
//   ;-----------------------------------------
//   @@ OV8859R1A Key 1632x1224_30FPS_MIPI_2LANE(Binning)
...
{
	LOG_INF("E\n");
//  @@ OV8859R2A Initial 1632x1224_30FPS_MIPI_2LANE(Binning)
//  100 99 1632 1224 ; Resolution
//  102 80 1
//  102 3601 bb8;30fps
//  102 40 0 ; HDR Mode off
...
//  ;;
	...
	if (ov8859version == OV8859R1A){
		LOG_INF("inter r1asensor_init() ov8859version = %d(0=r2a,1=r1a)\n",ov8859version); 
		r1asensor_init();
	}
...
//   100 99 1632 1224
	...
	write_cmos_sensor(0x380a, 0x04);     //
	write_cmos_sensor(0x380b, 0xc8);     //
	write_cmos_sensor(0x380c, 0x07);     //7
	write_cmos_sensor(0x380d, 0xa0);     //88
...
{
	LOG_INF("E\n");
//  @@ OV8859R2A VGA 120fps_PCLK=148.8M
//  100 99 640 480
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	kal_uint16 lens_id = 0; 
		...
		printk("[ryf][ov8859] get_imgsensor_id start !\n");	  
			...
			*sensor_id = *sensor_id + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				if((read_cmos_sensor(0x302A)) == 0XB2){
 				ov8859version = OV8859R2A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8859version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8859version);	
				break;
				}
				else if((read_cmos_sensor(0x302A)) == 0XB1){
 				ov8859version = OV8859R1A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8859version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8859version);	
				break;
				}
				else{
				LOG_INF("read ov8859 R1A R2A bate fail\n");	  
	...
	do {
		...
		if (lens_id == 0xd9) {				
		printk("[ryf][ov8859] init need id 0xd9 lens_id id: 0x%x\n", lens_id);	  
			break;
		}	
		printk("[ryf][ov8859] init need id 0xd9 Read lens_id  fail, lens_id: 0x%x\n", lens_id);
		retry--;
	} while(retry > 0);
		retry = 3;
	if (lens_id != 0xd9)
...
#endif	
		printk("[ryf][ov8859] get_imgsensor_id finished !\n");	  
	return ERROR_NONE;
}
...
{
	int RG_Ratio_Typical = 0x13B, BG_Ratio_Typical = 0x122;
...
static kal_uint32 open(void)
{
	...
	kal_uint16 lens_id = 0; 
	printk("[ryf][ov8859] open  enter and start !\n");	  
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			sensor_id = sensor_id +1 ;
			if (sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("[ryf][ov8859] i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			LOG_INF("[ryf][ov8859] Read sensor id fail:0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
	...
	do {
		lens_id = (read_cmos_sensor(0x7012) );
		if (lens_id == 0xd9) {				
		printk("[ryf][ov8859] open need id 0xd9 lens_id id: 0x%x\n", lens_id);	  
			break;
		}	
		printk("[ryf][ov8859] open need id 0xd9 Read lens_id  fail, lens_id: 0x%x\n", lens_id);
		retry--;
	} while(retry > 0);
		retry = 2;
	if (lens_id != 0xd9)
	...
	spin_unlock(&imgsensor_drv_lock);
	printk("[ryf][ov8859] open  end and finished !\n");	  
	return ERROR_NONE;
}	/*	open  */
...
UINT32 OV8859_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
...
UINT32 OV8859R2A_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
