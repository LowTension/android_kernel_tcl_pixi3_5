...
#include "ov5670_front_sunrise_mipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov5670_front_sunrise_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670_FRONT_SUNRISE_MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+0x000f;  //567e or 567f careful
			 module_id=ov5670_front_read_otp();
			 if(module_id==0x0c)sensor_id-=1;              //0x0c: sunrise  0x06 qtech  567e is sunrise,567f is qtech  wenzhiquan
...
UINT32 OV5670_FRONT_SUNRISE_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
