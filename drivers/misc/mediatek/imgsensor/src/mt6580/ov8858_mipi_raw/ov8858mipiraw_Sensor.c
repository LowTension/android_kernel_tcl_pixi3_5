...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre_delay_frame = 1, 		// old:3 enter preview delay frame num  jiangjingjing-modify-for-pixi3-55-goole-cts-test-Task:340217-20150612
	.video_delay_frame = 1,	//old:3  enter video delay frame num     jiangjingjing-modify-for-pixi3-55-goole-cts-test-Task:340217-20150612
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	kal_uint16 lens_id = 0; 
		printk("[ryf][ov8858] get_imgsensor_id start !\n");	  
	...
	do {
		...
		if (lens_id == 0x11) {				
		printk("[ryf][ov8858] open need id 0x11 lens_id id: 0x%x\n", lens_id);	  
			break;
		}	
		printk("[ryf][ov8858] open need id 0x11 Read lens_id  fail, lens_id: 0x%x\n", lens_id);
		retry--;
	} while(retry > 0);
		retry = 3;
	if (lens_id != 0x11)
	...
	printk("[ryf][ov8858] get_imgsensor_id finished !\n");	  
	return ERROR_NONE;
}
...
{
	int RG_Ratio_Typical = 0x106, BG_Ratio_Typical = 0x11E;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("[ryf][ov8858] i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			LOG_INF("[ryf][ov8858]Read sensor id fail, id: 0x%x,0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
