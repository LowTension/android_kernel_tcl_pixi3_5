...
#endif
//begin-20150619-modify-wenzhiquan-for DTV driver patch which offered by fci  d233922
	bbm_write(handle, 0x00b0, 0x07);
//	bbm_write(handle, 0x00b0, 0x03);
//end-20150619-modify-wenzhiquan-for DTV driver patch which offered by fci  d233922
...
s32 fc8180_scan_status(HANDLE handle)
{
	...
	for (i = 0; i < ifagc_timeout; i++) {
		...
		print_log(handle, "[FC8180] fc8180_scan_status ifagc_timeout 0x%x, %d\n", data, i);
		if (data & 0x01)
			break;
	...
	for (; i < ofdm_timeout; i++) {
		bbm_read(handle, 0x3025, &data);
		print_log(handle, "[FC8180] fc8180_scan_status ofdm_timeout 0x%x, %d\n", data, i);
	...
	for (; i < ffs_lock_timeout; i++) {
		...
		print_log(handle, "[FC8180] fc8180_scan_status ffs_lock_timeout 0x%x, %d\n", data, i);
	...
	for (i = 0; i < dagc_timeout; i++) {
		bbm_read(handle, 0x3026, &data);
		print_log(handle, "[FC8180] fc8180_scan_status dagc_timeout 0x%x, %d\n", data, i);
	...
	for (i = 0; i < cfs_timeout; i++) {
		bbm_read(handle, 0x3025, &data);
		print_log(handle, "[FC8180] fc8180_scan_status cfs_timeout 0x%x, %d\n", data, i);
	...
	for (i = 0; i < tmcc_timeout; i++) {
		bbm_read(handle, 0x3026, &data);
		print_log(handle, "[FC8180] fc8180_scan_status tmcc_timeout 0x%x, %d\n", data, i);
		if (data & 0x02)
			break;
	...
	print_log(handle, "[FC8180] fc8180_scan_status 0x4113(0x%x) 0x4114(0x%x)\n", data, data1);
	...
	for (i = 0; i < ts_err_free_timeout; i++) {
		...
		print_log(handle, "[FC8180] fc8180_scan_status ts_err_free_timeout 0x%x, %d\n", data, i);
		if (data & 0x01)
			break;
