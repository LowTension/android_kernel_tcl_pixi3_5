...
#include <mach/mt_clkbuf_ctl.h>	/*  for clock buffer 26M output */
#endif
...
#define GPIO_ISDBT_IRQ  20   //GPIO20,EINT4
#define GPIO_ISDBT_PWR_EN  GPIO_DTV_LDO_EN_PIN
#define CUST_EINT_ISDBT_NUM CUST_EINT_DTV_NUM
#define GPIO_ISDBT_EN_PIN_M_GPIO GPIO_DTV_LDO_EN_PIN_M_GPIO
#define CUST_EINT_ISDBT_TYPE EINTF_TRIGGER_FALLING  //CUST_EINTF_TRIGGER_FALLING

u8 static_ringbuffer[RING_BUFFER_SIZE];
// struct regulator *regisdbt = NULL;  //remove by wenzhiquan 20150512 for proto no regulor control

#define FC8180_HW_NAME  "fc8180power"
...
static irqreturn_t isdbt_irq(int irq, void *dev_id)
{
	isdbt_isr_sig = 1;
	wake_up_interruptible(&isdbt_isr_wait);
	print_log(0, "[FC8300] isdbt_irq %d, 0x%x\n", irq, dev_id);
    mt_eint_unmask(CUST_EINT_ISDBT_NUM);
	return IRQ_HANDLED;
}
#endif
...
/*begin-remove by wenzhiquan 20150512 for proto no regulor control
	print_log(0, "isdbt_hw_init\n");
	nRetVal = regulator_set_voltage(regisdbt, 1800000, 1800000); 
	if (nRetVal)
    {
        printk("FC8300]  Could not set to 1800mv.\n");
    }
     printk(" FC8300]  isdbt_hw_settingregulator_set_voltage ok !\n");
    regulator_enable(regisdbt);
end-remove by wenzhiquan 20150512 for proto no regulor control*/
	...
	mdelay(100);
	mt_set_gpio_out(GPIO_FM_TV_V1_PIN, GPIO_OUT_ONE);
	mt_set_gpio_out(GPIO_FM_TV_V2_PIN, GPIO_OUT_ZERO);
#else
...
#ifdef FEATURE_MTK
	mt_set_gpio_out(GPIO_ISDBT_PWR_EN, GPIO_OUT_ZERO);
	mt_set_gpio_out(GPIO_FM_TV_V1_PIN, GPIO_OUT_ZERO);
	mt_set_gpio_out(GPIO_FM_TV_V2_PIN, GPIO_OUT_ONE);
#else
	gpio_set_value(GPIO_ISDBT_PWR_EN, 0);
#endif
	mdelay(5);
//begin-remove by wenzhiquan 20150512 for proto no regulor control
//      regulator_disable(regisdbt);
//end-remove by wenzhiquan 20150512 for proto no regulor control
	mutex_unlock(&driver_mode_lock);
}
...
long isdbt_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_ISDBT_BYTE_READ:
		...
		print_log(hInit, "[FC8180] IOCTL_ISDBT_BYTE_READ 0x%x, %d\n", (u16)info.buff[0], (u8)info.buff[1]);
		break;
	case IOCTL_ISDBT_WORD_READ:
		...
		res = bbm_com_word_read(hInit, (u16)info.buff[0]
			, (u16 *)(&info.buff[1]));
		err |= copy_to_user((void *)arg, (void *)&info, size);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_WORD_READ 0x%x, %d\n", (u16)info.buff[0], (u16)info.buff[1]);
		break;
	case IOCTL_ISDBT_LONG_READ:
		...
		res = bbm_com_long_read(hInit, (u16)info.buff[0]
			, (u32 *)(&info.buff[1]));
		err |= copy_to_user((void *)arg, (void *)&info, size);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_LONG_READ 0x%x, %d\n", (u16)info.buff[0], (u32)info.buff[1]);
		break;
	case IOCTL_ISDBT_BULK_READ:
		...
		err |= copy_to_user((void *)arg, (void *)&info, size);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_BULK_READ 0x%x, Size %d, [0x%x]\n", (u16)info.buff[0], (u32)info.buff[1], (u32)info.buff[2]);
		break;
	case IOCTL_ISDBT_BYTE_WRITE:
		err = copy_from_user((void *)&info, (void *)arg, size);
		res = bbm_com_byte_write(hInit, (u16)info.buff[0]
			, (u8)info.buff[1]);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_BYTE_WRITE 0x%x, %d\n", (u16)info.buff[0], (u8)info.buff[1]);
		break;
	case IOCTL_ISDBT_WORD_WRITE:
		err = copy_from_user((void *)&info, (void *)arg, size);
		res = bbm_com_word_write(hInit, (u16)info.buff[0]
			, (u16)info.buff[1]);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_WORD_WRITE 0x%x, %d\n", (u16)info.buff[0], (u16)info.buff[1]);
		break;
	case IOCTL_ISDBT_LONG_WRITE:
		err = copy_from_user((void *)&info, (void *)arg, size);
		res = bbm_com_long_write(hInit, (u16)info.buff[0]
			, (u32)info.buff[1]);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_LONG_WRITE 0x%x, %d\n", (u16)info.buff[0], (u32)info.buff[1]);
		break;
	case IOCTL_ISDBT_BULK_WRITE:
		err = copy_from_user((void *)&info, (void *)arg, size);
		res = bbm_com_bulk_write(hInit, (u16)info.buff[0]
			, (u8 *)(&info.buff[2]), info.buff[1]);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_BULK_WRITE 0x%x, Size %d, [0x%x]\n", (u16)info.buff[0], (u8)info.buff[1], (u32)info.buff[2]);
		break;
	case IOCTL_ISDBT_TUNER_READ:
		...
		res = bbm_com_tuner_read(hInit, (u8)info.buff[0]
			...
			, (u8)info.buff[2]);
		err |= copy_to_user((void *)arg, (void *)&info, size);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_TUNER_READ 0x%x, %d\n", (u8)info.buff[0], (u8)info.buff[3]);
		break;
	case IOCTL_ISDBT_TUNER_WRITE:
		...
		res = bbm_com_tuner_write(hInit, (u8)info.buff[0]
			, (u8)info.buff[1], (u8 *)(&info.buff[3])
			, (u8)info.buff[2]);
		print_log(hInit, "[FC8180] IOCTL_ISDBT_TUNER_WRITE 0x%x, %d\n", (u8)info.buff[0], (u8)info.buff[3]);
		break;
	case IOCTL_ISDBT_TUNER_SET_FREQ:
		...
		print_log(hInit, "[FC8180] IOCTL_ISDBT_TUNER_SET_FREQ  %d\n", f_rf);
#ifndef BBM_I2C_TSIF
			mutex_lock(&ringbuffer_lock);
...
/*begin wenzhiquan 20150512 add for 26M output */
			/* enable nfc clock buffer */
			clk_buf_ctrl(CLK_BUF_NFC, 1);
/*begin wenzhiquan 20150512 add for 26M control */
...
/*begin-remove by wenzhiquan 20150512 for proto no regulor control
	regisdbt=regulator_get(fc8180_misc_device.this_device,PMIC_APP_DTV_POWER_D); // get pointer to regulator structure
	if (IS_ERR(regisdbt)) 
