...
s32 fci_i2c_init(HANDLE handle, s32 speed, s32 slaveaddr)
{
	...
	//printk(handle, "Internal I2C Pre-scale: 0x%02x\n", pr);
...
s32 fci_i2c_read(HANDLE handle, u8 chip, u8 addr, u8 alen, u8 *data, u8 len)
{
	...
	if (ret != I2C_OK) {
		//printk(handle, "fci_i2c_read() result=%d, addr = %x, data=%x\n", ret, addr, *data);
		return ret;
	}
...
s32 fci_i2c_write(HANDLE handle, u8 chip, u8 addr, u8 alen, u8 *data, u8 len)
{
	...
	if (ret != I2C_OK) {
		//printk(handle, "fci_i2c_write() result=%d, addr= %x, data=%x\n", ret, addr, *data);
	}
