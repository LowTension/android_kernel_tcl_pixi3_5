...
******************************************************************************/
kal_bool  Doubletap_Enable =KAL_FALSE;  //jiangjingjing-add-for-doubletap-20151014
...
static ssize_t show_doubletap(struct device *dev, struct device_attribute *attr, char *buf)
{	 
     int ret=0; 	 
     ret = sprintf(buf, "%d\n",value);     
     return ret;
}
static ssize_t store_doubletap(struct device* dev, struct device_attribute *attr, const char *buf, size_t count)
{    

	if (sscanf(buf, "%d", &value) != 1) 
	{
		printk("doubletap: Invalid values\n");
		return -EINVAL;
	}

       if(1==value)
    	{
          //do something about disable scan key
		printk("doubletap: 1\n");
              Doubletap_Enable = KAL_TRUE;
    	}
	else 
	{
              Doubletap_Enable = KAL_FALSE;   
	}
...
static DEVICE_ATTR(ctp, 0644, show_ctp, store_ctp);
	//begin: modify by fangjie for MMITest show camera info
//old: static DEVICE_ATTR(02_main_camera, 0644, show_main_camera, store__main_camera);
//old: static DEVICE_ATTR(03_sub_camera, 0644, show_sub_camera, store_sub_camera);
...
//jiangjingjing-add-for-doubletap-20151014-begin
static DEVICE_ATTR(doubletap,0644,show_doubletap,store_doubletap);
//jiangjingjing-add-for-doubletap-20151014-end
...
{   
	...
	//begin: modify by fangjie for MMITest show camera info
   //old:  if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_02_main_camera)) != 0) goto exit_error;
   //old:  if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_03_ub_camera)) != 0) goto exit_error;
	...
	//end: modify by fangjie for MMITest show camera info
    ...
    //kaka_14_1113 end
    //jiangjingjing-add-for-doubletap-20151014-begin
   if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_doubletap)) != 0) goto exit_error; 
	//jiangjingjing-add-for-doubletap-20151014-end
...
static int HardwareInfo_driver_remove(struct platform_device *dev)
{
    ...
    //begin: modify by fangjie for MMITest show camera info
    //old:device_remove_file(&(dev->dev), &dev_attr_02_main_camera); 
   //old: device_remove_file(&(dev->dev), &dev_attr_03_sub_camera);
   ...
   //end: modify by fangjie for MMITest show camera info
    ...
    //kaka_14_1113 end
    //jiangjingjing-add-for-doubletap-20151014-begin
    device_remove_file(&(dev->dev), &dev_attr_doubletap);
    //jiangjingjing-add-for-doubletap-20151014-end
