...
extern char *g_alsps_name;//jiangjingjing-add-for-pixi3-55-20150418
#endif
	  ...
	  if(0 != alsps_init_list[i])
	  {
		...
		if(0 == err)
		{
		   ALSPS_LOG(" alsps real driver %s probe ok\n", alsps_init_list[i]->name);
		    #if defined (CONFIG_HW_INFO) 
		     g_alsps_name = alsps_init_list[i]->name;//jiangjingjing-add-for-pixi3-55-20150418
		    #endif
		   break;
		}
	  }
...
int ps_register_control_path(struct ps_control_path *ctl)
{
	...
	//jiangjingjing-add-for-pixi3-55-20150906-begin
	...
	#endif
	//jiangjingjing-add-for-pixi3-55-20150906-end
