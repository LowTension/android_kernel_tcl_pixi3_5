...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.5.2 20150727"
...
/*----------------------------------------------------------------------------*/
	...
	#define STK_HT_N_CT	650
#endif /* #ifdef STK_TUNE0 */

#define STK_DEF_LOW_THD 2000
#define STK_DEF_HIGH_THD 2100
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
...
#ifndef CALI_EVERY_TIME
	if(obj->ps_stat_data[1]==0)
	{
		atomic_set(&obj->ps_high_thd_val, STK_DEF_HIGH_THD);  //obj->hw->ps_threshold_high
		atomic_set(&obj->ps_low_thd_val, STK_DEF_LOW_THD);  //obj->hw->ps_threshold_low
	}
	else
	{
	...
	if(obj->ps_stat_data[1]==0)
	{
		obj->ps_high_thd_boot = STK_DEF_HIGH_THD;  //obj->hw->ps_threshold_high
		obj->ps_low_thd_boot = STK_DEF_LOW_THD;  //obj->hw->ps_threshold_low	
	}
	else
	{
...
static int32_t stk_tune_zero_get_ps_data(struct stk3x1x_priv *obj)
{
	...
	if(obj->data_count == 5)
	{
		...
		#if defined(STK_MINISW_SHOW)
		boot_prox_mean = obj->ps_stat_data[1];
		#endif
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(ret == 0)
	{
		...
		#if defined(STK_MINISW_SHOW)
		boot_prox_mean = (boot_prox_mean+word_data)/2;
		#endif
...
static ssize_t show_ps_status_value(struct device_driver *ddri, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d:%d:%d\n", 
		boot_prox_mean, atomic_read(&stk3x1x_obj->ps_high_thd_val), atomic_read(&stk3x1x_obj->ps_low_thd_val));
	return len;
}
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_IOCTL_SET_CALI:                 
			...
			if(obj->ps_cali == 0)
			{
				APS_LOG("ALSPS_IOCTL_SET_CALI error : obj->ps_cali:%d high:%d low:%d\n", obj->ps_cali, 
													atomic_read(&obj->ps_high_thd_val), 
													atomic_read(&obj->ps_low_thd_val));
				goto err_out; 
			}
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	// add by lidianlong 2015-09-04 begin
	if(1 == obj->hw->polling_mode_ps)
	{
		wake_lock_init(&ps_lock,WAKE_LOCK_SUSPEND,"ps wakelock");
	}
	// add by lidianlong 2015-09-04 end 
	...
	// add by lidianlong 2015-09-04 begin
	if(1 == obj->hw->polling_mode_ps)
		ps_ctl.is_report_input_direct = false; //false => for Polling mode
	else
		ps_ctl.is_report_input_direct = true; 
