...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.5.2 20150601"
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
    ...
    if(obj->ps_stat_data[1] <obj->stk_max_min_diff)
    	{
	obj->psa = obj->ps_stat_data[0];
	obj->psi = obj->ps_stat_data[2];							
      if(obj->boot_cali <100)
      	{
      	obj->stk_ht_n_ct=70;
	obj->stk_lt_n_ct=30;	
	  }
	  else if(obj->boot_cali >500)
	  {
         obj->stk_ht_n_ct=120;
	  obj->stk_lt_n_ct=60;
	  }
	...
	atomic_set(&obj->ps_high_thd_val, obj->boot_cali  + obj->stk_ht_n_ct); 
	atomic_set(&obj->ps_low_thd_val, obj->boot_cali + obj->stk_lt_n_ct); 		
    	}
...
static int stk_ps_tune_done(struct stk3x1x_priv *obj)
{
	int err;
	...
	obj->psi = obj->ps_stat_data[2];
      ...
      if(obj->ps_stat_data[1]<100)
      	{
      	obj->stk_ht_n_ct=70;
	obj->stk_lt_n_ct=30;	
	  }
	  else if(obj->ps_stat_data[1] >500)
	  {
         obj->stk_ht_n_ct=120;
	  obj->stk_lt_n_ct=60;
	  }
...
static int32_t stk_tune_zero_get_ps_data(struct stk3x1x_priv *obj)
{
	...
	if(obj->data_count >= 7)
	{
		obj->ps_stat_data[1]  =(obj->ps_stat_data[1] -(obj->ps_stat_data[0] +obj->ps_stat_data[2] )) /(obj->data_count-2);	
		APS_LOG("%s: pscali =%d  stk_max_min_diff=%d\n", __func__, obj->ps_stat_data[1], obj->stk_max_min_diff);
		#if defined(STK_MINISW_SHOW)
		if(obj->tune_zero_init_proc ==true)
		{
			obj->boot_cali = obj->ps_stat_data[1];
		}
		#endif
		stk_ps_tune_zero_final(obj);
	       APS_LOG("%s: set HT=%d,LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val),  atomic_read(&obj->ps_low_thd_val));
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	{	
		obj->data_count = -1;
		stk_ps_tune_done(obj);
		return 0;	
	}
	
	if((ret = stk3x1x_read_ps(obj->client, &obj->ps)))
	{
		APS_ERR("stk3x1x read ps data: %d\n", ret);
		return ret;
	}	
...
static ssize_t show_ps_status_value(struct device_driver *ddri, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d:%d:%d\n", 
		stk3x1x_obj->boot_cali, atomic_read(&stk3x1x_obj->ps_high_thd_val), atomic_read(&stk3x1x_obj->ps_low_thd_val));
	return len;
}
...
static int ps_enable_nodata(int en)
{
    ...
    printk("stk3x1x_obj ps_enable_nodata enable value = %d\n", en);
	...
	if (en)
	{
		hrtimer_start(&stk3x1x_obj->ps_tune0_timer, stk3x1x_obj->ps_tune0_delay, HRTIMER_MODE_REL);	
	}
	else
		hrtimer_cancel(&stk3x1x_obj->ps_tune0_timer);	
#endif
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
