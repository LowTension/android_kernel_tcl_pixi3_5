...
static bool mPlaybackUseSram; //add by zyurong for task664143 20150923
...
static int mtk_pcm_dl1_open(struct snd_pcm_substream *substream)
{
    ...
    if (GetSramState() == SRAM_STATE_FREE)
    {
        ...
        mPlaybackUseSram = true; //add by zyurong for task664143 20150923
    }
    else
    {
        mtk_pcm_dl1_hardware.buffer_bytes_max = GetPLaybackDramSize();
        mPlaybackSramState = SRAM_STATE_PLAYBACKDRAM;
        mPlaybackUseSram = false; //add by zyurong for task664143 20150923
    }
    ...
    if (copy_size != 0)
    {
        ...
        if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) // copy once
        {
            ...
            else
            {
                ...
                //modified by zyurong for task664143 20150923
				if (mtk_local_audio_copy_from_user(mPlaybackUseSram,(Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp), data_w_ptr, copy_size))
                {
        ...
        else  // copy twice
        {
            ...
            else
            {
                ...
                //modified by zyurong for task664143 20150923
				if ((mtk_local_audio_copy_from_user(mPlaybackUseSram, (Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp), data_w_ptr , (unsigned int)size_1)))
                {
            ...
            else
            {
                ...
                //modified by zyurong for task664143 20150923
				if ((mtk_local_audio_copy_from_user(mPlaybackUseSram, (Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp), (data_w_ptr + size_1), size_2)))
                {
