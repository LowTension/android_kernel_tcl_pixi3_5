...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    else
    {
        ...
        mCaptureUseSram = false;//add by zyurong for task664143 20150923
    }
                                ...
                                void __user *dst, snd_pcm_uframes_t count)
{
    ...
    if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize)
    {
        ...
        if (mtk_local_audio_copy_to_user(mCaptureUseSram, (void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), read_size))	
		{
    ...
    else
    {
		...
		//modified by zyurong for task664143 20150923
        if (mtk_local_audio_copy_to_user(mCaptureUseSram, (void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), (unsigned int)size_1))
        {
		...
		//modified by zyurong for task664143 20150923
        if (mtk_local_audio_copy_to_user(mCaptureUseSram, (void __user *)(Read_Data_Ptr + size_1), (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr), size_2))
        {
